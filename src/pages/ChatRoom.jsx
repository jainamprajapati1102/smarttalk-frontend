...
//       console.log("msges -->>", response.data.messages);
...
const ChatRoom = () => {
  ...
  const [message, setMessage] = useState([]);
  const [skip, setSkip] = useState(0);
  const skipRef = useRef(0); // keep latest skip value accessible in handlers
  ...
  // Helper: safely compare ids (ObjectId or string)
  ...
  useEffect(() => {
    ...
    // join the chat room (server must socket.join(chatId))
    socket.emit("joinChat", selectedChat._id);

    // named handlers so cleanup can remove the same function references
    const handleTyping = ({ chatId, userId }) => {
      if (sameId(chatId, selectedChat._id) && !sameId(userId, loggedUser._id)) {
        setIsTypingOther(true);
      }
    };
    ...
    const handleNewMessage = (msg) => {
      // msg.chat might be { _id: ... } or just id
      const msgChatId = msg?.chat?._id ?? msg?.chat;
      if (!sameId(msgChatId, selectedChat._id)) {
        // Optionally update sidebar/unread counts here
        return;
      }
  ...
  }, [socket, selectedChat, loggedUser?._id]); // re-run when chat changes

  // ========= FETCH MESSAGES =========
  // fetchMessages now accepts explicit forSkip to avoid stale skip usage
  const fetchMessages = useCallback(
    async ({ append = false, prepend = false, forSkip = 0 } = {}) => {
      ...
      try {
        ...
        const newMessages = response.data.messages || [];
        // debug
        // console.log("Fetched messages:", newMessages.length, "forSkip:", forSkip);
  ...
  // when selectedChat changes: reset pagination and load
  useEffect(() => {
    if (!selectedChat?._id) {
      navigate("/");
      return;
  ...
  const handleScroll = useCallback(() => {
    ...
    // use threshold to avoid requiring exact 0
