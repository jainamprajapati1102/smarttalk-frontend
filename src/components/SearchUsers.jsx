...
const SearchUsers = ({ onBack }) => {
  ...
  // get logged user to determine the "other" participant for one-to-one chat
  ...
  const accessChat = async (userId) => {
    try {
      ...
      if (res.status === 201 || res.status === 200) {
        ...
        // set selected chat in context so ChatRoom shows it
        setSelectedChat(chat);

        // compute the user object to pass to ChatRoom (for 1:1 chats)
        let otherUser = null;
        if (Array.isArray(chat.users)) {
          otherUser =
            chat.users.find((u) => String(u._id) !== String(loggedUser?._id)) ||
            null;
        }

        // close search panel immediately (if onBack provided)
        if (typeof onBack === "function") onBack();

        // navigate to the chat route (absolute path) and pass user + chat in state
        navigate("/chat", { state: { user: otherUser, chat } });
      } else {
        console.error("Failed to access/create chat:", res);
      }
    } catch (err) {
      console.error("Chat access error:", err);
    }
